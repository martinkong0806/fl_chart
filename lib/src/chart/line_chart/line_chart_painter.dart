...
class LineChartPainter extends AxisChartPainter<LineChartData> {
  ...
  LineChartPainter() : super() {
    ...
    _barBarPaint = Paint()..style = PaintingStyle.fill;

    _barBarStrokePaint = Paint()..style = PaintingStyle.stroke;
  ...
  void paint(BuildContext context, CanvasWrapper canvasWrapper,
    ...
    for (var i = 0; i < data.lineBarsData.length; i++) {
      ...
      drawBarGroups(canvasWrapper, data.barGroups, holder);
    ...
    for (var i = 0; i < data.showingTooltipIndicators.length; i++) {
      ...
      if (showingBarSpots.isEmpty && tooltipSpots.showingBarGroups.isEmpty) {
        continue;
      }
      final barSpots = List<LineBarSpot>.of(showingBarSpots);
      FlSpot topSpot = FlSpot.zero;
      if (barSpots.isNotEmpty) {
        topSpot = barSpots[0];
      ...
      if (tooltipSpots.showingBarGroups.isNotEmpty) {
        barGroupIndex = tooltipSpots.showingBarGroups[0];
        showOnBarGroup = data.barGroups[barGroupIndex];
        showOnRodData = showOnBarGroup.barRods[0];

        drawBarGroupTouchTooltip(
            context,
            canvasWrapper,
  ...
  void drawBarGroups(
    ...
    for (var i = 0; i < barGroups.length; i++) {
      ...
      for (var j = 0; j < barGroup.barRods.length; j++) {
        ...
        final x = getPixelX(barGroup.x.toDouble(), viewSize, holder);
        ...
        if (barRod.backDrawRodData.show &&
          ...
          _barBarPaint.setColorOrGradient(
            backDraw.color,
            backDraw.gradient,
            barRRect.getRect(),
          );
          canvasWrapper.drawRRect(barRRect, _barBarPaint);
        }
        ...
        if (barRod.toY != barRod.fromY) {
          ...
          if (borderSide.width > 0 && borderSide.color.opacity > 0) {
            _barBarStrokePaint.color = borderSide.color;
            _barBarStrokePaint.strokeWidth = borderSide.width;
            canvasWrapper.drawRRect(barRRect, _barBarStrokePaint);
          }
          ...
          if (barRod.rodStackItems.isNotEmpty) {
            for (var i = 0; i < barRod.rodStackItems.length; i++) {
              ...
              _barBarPaint.color = stackItem.color;
              canvasWrapper.save();
              canvasWrapper
  ...
  void drawStackItemBorderStroke(
    ...
    _barBarStrokePaint.color = stackItem.borderSide.color;
    _barBarStrokePaint.strokeWidth =
        min(stackItem.borderSide.width, barThickSize / 2);
    canvasWrapper.drawRRect(strokeBarRect, _barBarStrokePaint);
  }
  ...
  List<TouchLineBarSpot>? handleSpotTouch(
  ...
  BarTouchedSpot? handleBarGroupTouch(
    Offset localPosition,
    Size viewSize,
    PaintHolder<LineChartData> holder,
  ) {
    ...
    if (targetData.lineBarsData.isEmpty) {
      return null;
    }

    for (var i = 0; i < targetData.lineBarsData.length; i++) {
      for (var j = 0; j < targetData.barGroups.length; j++) {
        for (var k = 0; k < targetData.barGroups[j].barRods.length; k++) {
          final barWidth = targetData.barGroups[j].barRods[k].width;
          final halfBarWidth = barWidth / 2;
          double barTopY;
          double barBotY;

          final isUpward = targetData.barGroups[j].barRods[k].isUpward();
          if (isUpward) {
            barTopY = getPixelY(
              targetData.barGroups[j].barRods[k].toY,
              viewSize,
              holder,
            );
            barBotY = getPixelY(
              targetData.barGroups[j].barRods[k].fromY,
              viewSize,
              holder,
            );
          } else {
          ...
          final backDrawBarY = getPixelY(
              targetData.barGroups[j].barRods[k].backDrawRodData.toY,
              viewSize,
              holder);
          final touchExtraThreshold =
              targetData.barTouchData.touchExtraThreshold;

          final isXInTouchBounds = (touchedPoint.dx <=
                  getPixelX(targetData.barGroups[j].x.toDouble(), viewSize,
                          holder) +
                      halfBarWidth +
                      touchExtraThreshold.right) &&
              (touchedPoint.dx >=
                  getPixelX(targetData.barGroups[j].x.toDouble(), viewSize,
                          holder) -
                      halfBarWidth -
                      touchExtraThreshold.left);
          ...
          if (isXInTouchBounds && isYInTouchBounds) {
            final nearestGroup = targetData.barGroups[j];
            final nearestBarRod = nearestGroup.barRods[k];
            final nearestSpot =
                FlSpot(nearestGroup.x.toDouble(), nearestBarRod.toY);
            final nearestSpotPos = Offset(
                getPixelX(
                    targetData.barGroups[j].x.toDouble(), viewSize, holder),
                getPixelY(nearestSpot.y, viewSize, holder));
            ...
            return BarTouchedSpot(nearestGroup, i, nearestBarRod, j,
                touchedStack, touchedStackIndex, nearestSpot, nearestSpotPos);
          }
        }
      }
    }
  ...
  void drawBarGroupTouchTooltip(
    ...
    final barOffset = Offset(
      getPixelX(showOnBarGroup.x.toDouble(), viewSize, holder),
